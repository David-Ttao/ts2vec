...
for random_seed in $(seq 2021 2024); do
    for ((i=0; i<${#dataset_list[@]}; i++)); do
        for ((j=i+1; j<${#dataset_list[@]}; j++)); do
            for ((k=j+1; k<${#dataset_list[@]}; k++)); do
                for ((q=k+1; q<${#dataset_list[@]}; q++)); do
                    target1=${dataset_list[i]}
                    target2=${dataset_list[j]}
                    target3=${dataset_list[k]}
                    target4=${dataset_list[q]}
                    dset_pretrain=${target1}_${target2}_${target3}_${target4}
